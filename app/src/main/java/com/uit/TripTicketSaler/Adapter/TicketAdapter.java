...
public class TicketAdapter extends RecyclerView.Adapter<TicketAdapter.CarViewHolder> {
    ...
    public TicketAdapter(ArrayList<Coach> lCoach, int start, int end, int p, int c) {
        this.lCoach = lCoach;
        this.start = start;
        this.end = end;
        this.p = p;
    ...
    public void onBindViewHolder(@NonNull CarViewHolder holder, int position) {
        ...
        String pickUp = date.get(Calendar.HOUR_OF_DAY) + "giờ, ngày "
                +date.get(Calendar.DAY_OF_MONTH) + "/"
                +(date.get(Calendar.MONTH)+1) + "/"
                + date.get(Calendar.YEAR);
        holder.timePickUp.setText("Khởi hành: " + pickUp);
